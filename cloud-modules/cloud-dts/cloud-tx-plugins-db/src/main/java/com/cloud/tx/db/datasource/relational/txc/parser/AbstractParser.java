...
import com.cloud.tx.db.datasource.relational.txc.TxcPreparedStatement;
...
public abstract class AbstractParser<T>
{
    ...
    public CommitInfo parse(TxcStatement txcStatement) throws SQLException
    {
        ...
        T sqlParseStatement = (T) new MySqlStatementParser(sql)
                .parseStatement();
        ...
        {
            commitInfo.setSqlParams(
                    ((TxcPreparedStatement) txcStatement).getParamsList());
            commitInfo.setWhereParams(getWhereParams(
                    ((TxcPreparedStatement) txcStatement).getParamsList(),
                    sqlParseStatement));
        }

        // 解析之前的值
        commitInfo.setOriginalValue(getOriginValue(commitInfo.getWhereParams(),
                sqlParseStatement, txcStatement.getConnection()));

        // 解析之后的值
        commitInfo.setPresentValue(
                getPresentValue(commitInfo.getSqlParams(), sqlParseStatement));

        return commitInfo;

    }

    protected abstract List<Object> getWhereParams(List<Object> sqlParamsList,
            T parseSqlStatement);

    protected abstract String getWhere(T parseSqlStatement);

    // 从当前sql取出值
    public abstract TxcTable getPresentValue(List<Object> sqlParamsList,
            T parseSqlStatement);

    // 从数据库取出值
    public TxcTable getOriginValue(List<Object> whereParamsList,
            T parseSqlStatement, Connection connection) throws SQLException
    {
        ...
        String primaryKeyName = TableMetaUtils
                .getTableMetaInfo(connection, getTableName(parseSqlStatement))
                .getPrimaryKeyName();
        ...
        List<TxcLine> txcLines = ResultConvertUtils
                .convertWithPrimary(resultSet, primaryKeyName, getSqlType());
    ...
    protected abstract String selectSql(T parseSqlStatement,
