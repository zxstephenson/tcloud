/**
 * <p>文件名称: CompensateServiceImpl.java</p>
 * <p>项目描述: JROS 捷容平台 V3.1</p>
 * <p>公       司: 金证财富南京科技有限公司</p>
 * <p>版权所有: 版权所有(C)1998-2018</p>
 */
package com.cloud.tm.compensate.service.impl;
...
public class CompensateServiceImpl implements CompensateService
{
    ...
    public boolean saveCompensateMsg(final TransactionCompensateMsg transactionCompensateMsg)
    {
        ...
        log.info("Compensate->" + json);

        final String compensateKey = compensateDao.saveCompensateMsg(transactionCompensateMsg);
        System.err.println(this.getClass().getName() + "====>compensateKey======>" + compensateKey);
        ...
        threadPool.execute(new Runnable()
        {
            ...
            public void run()
            {
                try
                {
                    ...
                    log.error("Compensate Callback Address->" + url);
                    System.err.println("Compensate Callback Address->" + url);
                    String res = HttpUtils.postJson(url, requestJson.toJSONString());
                    log.error("Compensate Callback Result->" + res);
                    System.err.println("Compensate Callback Result->" + res);
                ...
                {
                    log.error("Compensate Callback Fails->" + e.getMessage());
                }
            }
        });
    ...
    @Override
    public void autoCompensate(final String compensateKey,
            TransactionCompensateMsg transactionCompensateMsg)
    {
        final String json = JSON.toJSONString(transactionCompensateMsg);
        log.info("Auto Compensate->" + json);
        // 自动补偿业务执行...
        ...
        try
        {
            ...
            log.info("Automatic Compensate Result->" + autoExecuteRes + ",json->" + json);
            while (!autoExecuteRes)
            {
                log.info("Compensate Failure, Entering Compensate Queue->"
                        + autoExecuteRes + ",json->" + json);
                executeCount++;
                if (executeCount == 3)
                {
        ...
        {
            log.error("Auto Compensate Fails,msg:" + e.getLocalizedMessage());
            // 推送数据给第三方通知
            autoExecuteRes = false;
        }
        ...
        String url = configReader.getCompensateNotifyUrl();
        log.error("Compensate Result Callback Address->" + url);
        String res = HttpUtils.postJson(url, requestJson.toJSONString());
        log.error("Compensate Result Callback Result->" + res);
    ...
    public List<TxModel> loadCompensateByModelAndTime(String path)
    {
        ...
        for (String json : logs)
        {
            ...
            model.setTime(DateUtil.formatDate(new Date(currentTime),
                    DateUtil.FULL_DATE_TIME_FORMAT));
    ...
    public void reloadCompensate(TxGroup txGroup)
    {
        ...
        if (compensateGroup != null)
        {
            ...
            if (compensateGroup.getList() != null
                    && !compensateGroup.getList().isEmpty())
            {
                // 引用集合 iterator，方便匹配后剔除列表
                Iterator<TxInfo> iterator = Lists
                        .newArrayList(compensateGroup.getList()).iterator();
                for (TxInfo txInfo : txGroup.getList())
                {
                    while (iterator.hasNext())
                    {
                        TxInfo cinfo = iterator.next();
                        if (cinfo.getModel().equals(txInfo.getModel()) && cinfo
                                .getMethodStr().equals(txInfo.getMethodStr()))
                        {
                            // 根据之前的数据补偿现在的事务
        ...
        log.info("Compensate Loaded->" + JSON.toJSONString(txGroup));
    }
    ...
    private boolean _executeCompensate(String json) throws ServiceException
    {
        ...
        String res = managerSenderService.sendCompensateMsg(
                modelInfo.getChannelName(), groupId, data, startError);

        log.debug("executeCompensate->" + json + ",@@->" + res);
